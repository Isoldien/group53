                            ...
                            <a href="{{ route('products.show', $product->product_id) }}" class="block">
                                <div class="h-48 bg-gray-200 dark:bg-gray-700 relative overflow-hidden group">
                                    @if($product->image_url)
                                        <img src="{{ filter_var($product->image_url, FILTER_VALIDATE_URL) ? $product->image_url : asset($product->image_url) }}" alt="{{ $product->product_name }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" onerror="this.onerror=null; this.src='{{ asset('images/placeholder1.png') }}';">
                                    @else
